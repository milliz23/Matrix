...
namespace Matrix
{
    class writeread
    {
        public static void ReadData(string path, ref int[,] matrix)
        {
            ...
            matrix = new int[lines.Length, strLen.Length]; //Инициализируется массив, куда будут записываться значения
            foreach (var line in lines)
            {
                ...
                {
                    matrix[k, j] = Convert.ToInt32(str[j]); //Передача в основной массив
                }
                k++;
            }
        }
